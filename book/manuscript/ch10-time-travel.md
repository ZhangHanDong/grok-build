...
**guard 家族：RAII 不只是释放资源，也是一笔"默认回滚"的小事务。** 第 4 章见过
cancel-on-drop、第 19 章见过 `AdmitGuard` 的名额归还；worktree 与 auth 里还有更完整的
一档——把"回滚"设成默认、"提交"要显式：

- `PartialWorktreeGuard` 在 drop 时**默认删除**这个还没建完的 worktree，除非成功后显式
  `disarm()`（crates/codegen/xai-fast-worktree/src/worktree/execute.rs:24、38）。于是 `?`
  早退、取消、panic 三条异常路径**全都走回滚**——你不必在每个出口手写清理，清理是"不
  disarm"时的默认结局。
- 更细的一手在 auth：`AuthSingleFlight` 的结束 guard 把 `generation` 一起带进 drop
  （crates/codegen/xai-grok-shell/src/auth/single_flight.rs:53）——一个被中止的旧登录任务
  在 Drop 时**不能清掉新登录任务的状态**，因为它带的 generation 已过期。清理动作带身份，
  避免"迟到的 Drop"误伤后继状态。

收束：**默认回滚、成功后 disarm；而当清理可能迟到、可能撞上后继者时，清理动作还得带上
generation 或身份**——这比"Drop 里释放资源"更进一步，是把一段时序逻辑做成了一笔有回滚、
有提交、有身份的小事务。
