...
## 9.8 编辑不是终点：LSP 诊断如何闭合反馈循环

前面五节都在讲**怎么把字节改对**——search_replace、hashline、apply_patch 各有各的
精确与善后。但改完之后呢？人类程序员改完一行会瞥一眼编辑器里的红波浪线；agent 改完
`search_replace` 只收到一句"成功"，对"这行改出编译错误了吗"一无所知。`lsp` 工具补的
正是这一环：把文本编辑闭合成一条**语义反馈循环**。

```mermaid
flowchart LR
    E[search_replace 编辑] --> N[notify_file_changed]
    N --> LS[语言服务器<br/>按扩展名路由]
    LS --> D[诊断 error/warning]
    D --> R["&lt;system-reminder&gt; 回喂"]
    R --> M[模型看见错误·再改]
    B["bash / git 改动"] -. 绕过 .-> LS
```

它经语言服务器提供六种代码智能操作（types.rs:82）：`GoToDefinition` / `FindReferences` /
`Hover` / `GoToImplementation` / `DocumentSymbol` / `WorkspaceSymbol`。但工具接口只是
表面，真正的工程在**生命周期管理**：

- 一个 `LspManager` 按**文件扩展名路由**到不同语言服务器（manager.rs:1）——多语言仓库里
  `.rs` 走 rust-analyzer、`.ts` 走 tsserver，互不干扰；
- **诊断作为提醒回喂模型**：收集到的 error/warning 包进 `<system-reminder>` 注入下一轮
  （manager.rs:295）——这正是第 8 章"报错文本是带内提示词"在编辑闭环里的落地，agent
  改完就看见自己改出的编译错误；
- 语言服务器会崩，于是有一套 crash-restart（restart.rs:1）：崩溃后自动重启，重启时
  **重放已跟踪的文档**让服务器重建状态（restart.rs:30），并用 `lifecycle_id` 把**旧实例
  的陈旧诊断**排除掉（manager.rs:24、restart.rs:23）——不让一个已死进程的过时报错误导模型。

**但有一个必须写出的闭环缺口。** 这套反馈只在**编辑经过 `search_replace`** 时才触发——
`notify_file_changed` 的源码注释白纸黑字："Only called after `search_replace`; other
mutations (bash, git) are not tracked"（manager.rs:185）。模型用 `bash` 跑 `sed -i`、或
`git checkout` 换了分支，这些改动**不进 LSP 的视野**，语义反馈就此断掉。这不是 bug，
是一条清晰的能力边界：**闭环只覆盖它自己知道的那条编辑路径**，绕过它的编辑也绕过了
整个反馈。

这一节和第 18 章的 tree-sitter codebase-graph 恰成对照：LSP 是**活的**语言服务语义
（实时、精确，但要跑起服务器、且只跟自己的编辑路径），codebase-graph 是**轻量静态**
索引（离线、近似，但对任意来源的文件一视同仁）。一活一静，各补对方的短板——这也是
atlas 三层 provenance 里"语法基线 always-on、语义层 opt-in"的同一权衡。
...
- LSP 闭合语义反馈：六操作、扩展名路由、诊断回喂 system-reminder、崩溃重放+lifecycle
  id 排陈旧；闭环缺口(只跟 search_replace，bash/git 逃逸)；活语义 vs 静态图(第18章) → 9.8
- 四个可迁移模式：error-as-prompt、归一后验证、新鲜度谱系、消歧三选一 → 9.9
...
> codex,opencode,lsp}。codex 原生环境描述基于 openai/codex 2026 年年中 main 分支。
