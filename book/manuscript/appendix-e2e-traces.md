...
到齐才显示，而是**边到边渲**。冻结的粒度是 **top-level 块而非行**：一个段落、
标题、闭合的代码块、表格一旦结束**且其后出现空行**，就被 checkpoint 冻结、用
syntect 上色；只有那**一个仍打开的尾块**在每个新 token 到来时重渲
（crates/codegen/xai-grok-markdown/src/lib.rs）。**接缝的设计意图**：冻结已定稿的块、
只重渲开放的尾块，把重渲成本从 O(N²) 压到 O(N)——已冻结的块不再参与解析
（详见第 15 章）。
...
**第 5 站 · checkpoint 记录改动（第 10 章）。** 改动被记入 checkpoint 体系，底层是
高性能 CoW worktree（crates/codegen/xai-fast-worktree/src/lib.rs）。注意 checkpoint
的粒度是 **prompt 边界而非每次编辑**：一个 prompt 内的所有改动作为一个 rewind 点
整体打包（第 10 章 `on_turn_boundary`），回滚回到的是某个 prompt **之前**的状态。
**接缝的设计意图**：checkpoint 是 agent 敢于"大胆行动"的安全网——因为整轮改动能
作为一个单元撤销，agent 与用户都能容忍它试错
（详见第 10 章）。
...
## 追踪三：一个后台 monitor 唤醒 agent 去处理外部事件

**用户动作**：用户让 agent"盯着 CI 的日志，出错了就修"。agent 起一个 `monitor`
后台脚本，然后继续做别的；当日志刷出一行错误，这行事件把 agent 从空闲里唤醒、送它
去处理，处理完更新目标进度。这是前面工具补齐章节（第 4 章时间维度、第 8 章媒体/
控制面工具）串起来的第一条**异步**路径。

**穿过的子系统**：第 8 章（monitor 作为工具注册）→ 第 4 章（时间维度：后台任务与
限流）→ 第 3 章（SessionActor 消息循环被事件唤醒）→ 第 6 章（durable 状态跨重启
存活）→ 第 4 章（模型处理事件、更新 goal）→ 第 13/16 章（事件在 TUI 里渲染）。

```mermaid
sequenceDiagram
    participant U as 用户
    participant MO as monitor 工具<br/>(ch8)
    participant BG as 后台脚本
    participant RL as 限流<br/>(ch4)
    participant SA as SessionActor<br/>(ch3)
    participant SM as sampler 循环<br/>(ch4)
    U->>MO: 起一个后台 monitor
    MO->>BG: spawn 长活管线
    ...
    RL->>SA: 限流后送通知（背压）
    SA->>SM: 空闲被唤醒，事件入队
    SM->>SM: 模型处理 + update_goal
    SM-->>U: 结果渲染回 TUI (ch13/16)
```

**第 1 站 · monitor 作为工具起一条后台管线（第 8 章）。** `monitor` 和 edit、bash
一样是注册在工具层的普通工具，但它 spawn 的是一条**长活管线**而非一次性调用——把
后台脚本的每一行 stdout 变成一条会话事件（monitor/tool.rs:30）。**接缝的设计意图**：
把"订阅外部事件"做成一个普通工具调用，agent 无需任何特殊 API 就获得"关注未来"的
能力（详见第 8 章工具抽象、第 4 章时间维度）。

**第 2 站 · 限流与背压（第 4 章）。** 日志可能瞬间刷出几百行。事件不直灌，而是过一个
token bucket + suppression：持续过载会自动杀掉 monitor、被抑制的事件计数并在恢复时
补一条通知（rate_limiter.rs:44）。**接缝的设计意图**：一个能唤醒 agent 的外部源必须
自带背压——否则一次日志洪泛就能把上下文冲爆、把成本烧光。

**第 3 站 · SessionActor 被事件唤醒（第 3 章）。** 事件经通知系统送到 `SessionActor`
的消息循环。第 3 章讲过：所有对会话状态的改动都串行经过这一个 actor。monitor 事件
在这里和用户输入、工具结果排在同一条队列上——**它就是又一种"有事发生了"的消息**。
**接缝的设计意图**：正因为一切都过同一个 actor，一个后台线程的异步事件才能安全并入
会话，不需要额外的锁（详见第 3 章）。

**第 4 站 · 模型处理并更新目标（第 4 章）。** 被唤醒后，sampler 循环把事件作为新输入
喂给模型；模型决定怎么应对（"这是个测试失败，我去修"），并可能调 `update_goal` 汇报
进度——而这一步会阻塞到 SessionActor 的裁决（第 4 章：声称完成 ≠ 系统接受，
update_goal/mod.rs:1，详见 4.4）。durable 的 scheduler / monitor 状态还能跨重启存活
（第 6 章）。

**第 5 站 · 渲染回终端（第 13/16 章）。** 事件与模型的后续动作经渲染管线出现在 TUI 里，
用户看着这一切自动发生。

**这条追踪揭示了什么。** 一个本质"请求-响应"的 agent，如何获得"关注未来"的能力——
**不是给循环加一个特殊的事件入口，而是把外部事件降维成"又一条经过 SessionActor 的
消息"**。monitor 把世界的变化翻译成事件、限流负责背压、SessionActor 负责串行归并、
goal 负责验证进度：每一层都在复用已有机制，没有一处是为"异步"新造的特例。这正是
actor 模型最大的红利——异步不是一个需要特殊照顾的边角，而是主干道上的又一种消息。
...
- **迁移到你自己的 agent**：这三条路径是终端 AI 编程代理的**通用骨架**。你在设计
