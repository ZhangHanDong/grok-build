...
## 17.3 工具太多时：把工具表变成可搜索的二级存储

MCP 把外部工具接进来（17.2），但引出一个模型侧的新问题：一个装了十几个 MCP
server 的会话可能有几百个工具。把它们的完整 JSON schema 每一轮都塞进采样上下文，
既烧 token，又有一个更隐蔽的代价——工具集一变，KV cache 就作废。

Grok Build 的答案是**不全量暴露**。采样时只发送内置工具定义，MCP 工具被排除在外：
`prepare_tool_definitions_inner` 调的是 `tool_definitions_builtins_only()`
（sampler_turn.rs:149）。模型长期只看见两个稳定入口：

```mermaid
flowchart LR
    Q["search_tool（创建 Linear issue）"] --> IDX[BM25 检索隐藏工具表]
    IDX --> R[返回 name / desc / input_schema]
    R --> U["use_tool（linear__save_issue, ...）"]
    U --> D[动态分发到真正的 MCP 工具]
```

**`search_tool` 是按需检索。** 先走精确名快路径——模型若已知限定名
（`grafana-ai__SearchDashboards`）或裸名，直接命中、跳过 BM25（tool_index.rs:169）；
否则按关键词 BM25 检索隐藏工具表，返回工具名、描述与完整 input schema
（search_tool/mod.rs:1、297）。它用 FNV-1a 而非默认 hasher，好让检索结构跨 Rust
版本与构建 profile 稳定（search_tool/mod.rs:34）。

**`use_tool` 是动态分发。** 拿着 `search_tool` 给的 schema 构造参数，`use_tool`
把调用转发到真正的 MCP 工具；它读 `ToolCallContext` 里的 `InnerDispatch`、绕开外层
`ToolBridge` mutex 避免死锁（同 8.5 的 InnerDispatch 防死锁一脉）。

这一层的价值，源码注释说得最直白：常驻这两个元工具，"让工具集跨 turn 保持稳定——
新的 MCP 工具上线时不会打断 KV cache"（use_tool/mod.rs:76）。于是三件事一次解决：
上下文成本（几百个 schema 不进每轮）、缓存稳定性（工具表变动不 invalidate 前缀）、
动态扩展（MCP 工具随时上下线，模型侧入口不变）。提炼成一条通用模式：

> 全部能力 ≠ 全部进入上下文。常驻元工具 + 按需检索 schema + 二次动态分发——把"工具
> 表"从上下文里的常量，变成一个可搜索的二级存储。

17.2 管"怎么把 MCP 安全地隔离进来"，这一节管"隔离进来的几百个工具，怎么才不淹没
模型、不烧缓存"——任何"能力远多于单轮上下文能容纳"的 agent 都会撞上这个问题。

## 17.4 Hooks：生命周期拦截与 fail-open 的坦白
...
> xai-grok-shell、xai-grok-tools（search_tool/use_tool）。
