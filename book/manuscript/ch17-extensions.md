...
泄露不牵连另一处，爆炸半径被限制在单个服务。写入时 Unix 下临时文件**开局即 0600**（无"先创建世界可读、再 chmod"的 TOCTOU（检查与使用之间的时间差）窗口，crates/codegen/xai-grok-mcp/src/credentials.rs:169），temp+rename 原子替换，
