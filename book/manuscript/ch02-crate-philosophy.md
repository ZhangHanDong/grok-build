...
**维度二：`codegen/` 目录 —— 这是全章要单独点出的"意外"。** 直觉会以为
...
但要精确读这行注释：它说的是 **workspace 根被自动生成**，紧接着还写着
"Prefer editing per-crate Cargo.toml files"——也就是**各 crate 自己的 `Cargo.toml`
是手工维护的**，被生成的只是 workspace 根（那份 79 行成员清单 + 集中依赖声明）。
结合 README（本仓库"periodically synced from the SpaceXAI monorepo"、用 `SOURCE_REV`
记录 monorepo commit）可以推断：根清单是 monorepo 构建系统的投影，人不手写它。
所以 `codegen/` 这个名字标记的不是"生成的代码"，而是"其成员关系由上游构建系统
登记、随 monorepo 同步刷新"的那批 crate。省掉的是**手工维护成员列表与版本对齐**
这层跨 crate 账，不是省掉每个 crate 的清单本身。

**维度三：`-types` / `-api` / `-core` / `-base` 后缀 —— 职责切面与意图标记。**
后缀标记了一个 crate 在"数据—逻辑—接线"谱系上*意图中*的位置：`-types` 意在承载
纯数据/wire 契约，`-core` 是核心逻辑，`-base` 是被抽出的基础模块，`-api` 是对外
接口面。但后缀只声明意图、不被 Cargo 强制——下一节会看到，正是 `-types` 这个后缀，
既承载了 workspace 里最反复出现的一条设计手法，也暴露了"意图未必等于不变量"。

## 2.5 依赖倒置：把纯数据切成独立 crate

Grok Build 的 workspace 里反复出现一种依赖边界手法：把共享类型放进独立的 `*-types`
crate。`crates/` 下有五个这样的 crate：`xai-grok-config-types`、`xai-grok-sampling-types`、
`xai-grok-workspace-types`、`xai-hooks-plugins-types`、`xai-tool-types`（`prod/mc/
cli-chat-proxy-types` 是第六个同类，因来自生产服务侧、在 2.3 已单列，此处不计）。
但这五个 crate 并不处在同一种纯度上：`xai-grok-workspace-types` 与
`xai-hooks-plugins-types` 接近轻量的 wire-contract crate，`xai-tool-types` 也基本
保持轻依赖；`xai-grok-config-types` 与 `xai-grok-sampling-types` 则已经吸收了实现
层依赖和行为逻辑。所以这不是一条被统一强制的规则，而是一条**反复出现、纯度不一的
架构方向**——它们的 `lib.rs` 顶注把"为什么要这样拆"写成了教科书，但 manifest 才
说明每个 crate 实际走到了哪一步。

先看最能说明"后缀是意图、不是不变量"的 `xai-grok-sampling-types`：
...
这段注释里"no I/O"那一半，从当前源码看基本成立：它并不创建 HTTP 客户端、不发请求、
不碰文件系统，对 `reqwest` 的使用止于错误类型与 `StatusCode` 判断。但"能被下游安全
依赖而不背上实现"这另一半，它并没有兑现——manifest 直接依赖 `reqwest`、`async-openai`、
`xai-grok-tools`、`xai-grok-compaction`（Cargo.toml:9-17），还对外 re-export `async_openai`
与 `xai-grok-tools` 的类型、并为 `ConversationItem` 实现 `xai-grok-compaction` 的 trait
（lib.rs:28、types.rs:396、conversation.rs:1176）。所以它不是"几乎零依赖的纯数据叶子"：
`-types` 后缀表达的是设计*意图*，却不是 Cargo 自动维护的不变量；真正的依赖边界，仍要
靠 manifest、架构测试和代码审查一起守住。相比之下，下面两个才是更干净的对照。
...
1. **给共享契约一个独立归属（并在合适的依赖图里防环）**：pager 需要认识插件/hook 的
   DTO，但不必把 shell 的内部领域类型当成 wire contract 来依赖——DTO 有了
   `xai-hooks-plugins-types` 这个独立的家，其他消费者也能只依赖协议类型。需要说清的是：
   `xai-grok-pager` 当前确实*直接依赖* `xai-grok-shell`（Cargo.toml:98），所以这里并不是
   "已经被打破的 shell/pager 环"；准确说，是这种把 DTO 下沉的手法，能在合适的依赖图里
   *防止或打破*循环——当前 shell/pager 之间尚未构成这样的环。
...
- **清单维护成本**：真正累人的不是每个 crate 那份薄清单，而是 79 行成员登记与
  跨 crate 的版本对齐。对冲手段是 2.4 说的**生成的 workspace 根**（Cargo.toml:1）
  ——成员清单与集中依赖由 monorepo 构建系统登记、随同步刷新，人不手写这份根清单
  （各 crate 自己的 Cargo.toml 仍是手编的）。
...
真正的差异不在**粒度**，而在 **workspace 根从哪来**：

- **Grok**：workspace **根** `Cargo.toml`（成员清单 + 集中依赖）是从 monorepo
  自动生成的投影（Cargo.toml:1 的 "Auto-generated workspace root"，同一行还叮嘱
  "per-crate 手动编辑"）。根随同步刷新，人不手写它；各 crate 自己的清单仍手编。
- **codex**：整个 workspace 连根带成员都是**原生手写**的 Cargo 项目，没有上游
  生成层。它上百个
...
- 四个顶层分区：codegen（产品闭包）/common（复用叶子）/build/prod，按"距核心远近
  + 复用范围"分层 → 2.3
- 三套正交命名维度：`grok` 前缀=复用范围、`codegen/`=成员由生成的 workspace 根登记、`-types/-core`
  后缀=*意图角色*（纯度不一，非 Cargo 强制）；看名即知*意图*层次 → 2.4（Cargo.toml:1）
- 依赖边界手法：五个 `-types` crate 把共享契约下沉，但*纯度不一*——真正轻依赖的买到
  快编 + 稳 API，`sampling-types`/`config-types` 则已吸收实现依赖，印证"后缀是意图、
  非不变量" → 2.5（workspace-types/lib.rs:3-6、hooks-plugins-types/lib.rs:4-9、
  sampling-types/Cargo.toml:9-17）
- 收益（并行/增量编译显式设计、统一 API 面）与代价（清单维护、版本漂移）各有对冲：
  生成的 workspace 根 + 集中式 `[workspace.dependencies]` + 分层 profile → 2.6
...
- codex 对照：codex 拆得更细（约 128 成员 > Grok 75），两家都推到上百 crate；真正
  差异是 workspace 根从哪来——Grok 的根是生成投影、codex 全手写，证明手写也能撑 100+ crate
  → 2.8
...
> 本章分析基于快照 SOURCE_REV 2ec0f0c（发布仓库同步 commit 8adf901，2026 年 7 月）的 workspace 结构。crate 数量
