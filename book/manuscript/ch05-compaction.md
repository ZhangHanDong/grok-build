...
1. **保守阈值**：`should_auto_compact`
   ...
   `auto_compact_threshold_percent`（默认 85%）时触发。要说清这个余量的
...
精确 tokenizer 不是做不了，是有前提地不划算：估算的消费场景全是"要不要压缩"
这类粗粒度决策——**在 bytes/4 大致成立的代码/英文负载下**，阈值决策对几个百分点
的估算误差不敏感，不翻盘。但这个"够用的粗糙"有明确的失效边界，就是上面点出的
CJK 系统性低估：那里的误差不是几个百分点，而是两三倍，足以让"自以为 60%"实则
已近溢出。所以边界之外真正兜底的不是更精的 tokenizer（它还要跟模型版本走、处理
多模态、付 CPU），而是 preflight 溢出预检与输入梯降这两层结构性防线——**用便宜的
估算 + 兜底闸门，替代昂贵的精确**。
