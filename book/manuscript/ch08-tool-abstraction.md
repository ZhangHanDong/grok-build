...
**错误枚举首先服务于调用方的下一步，其次才服务于 Display。** `ToolError` 把错误做成
"分类的词汇表"，这个思路还有更强的形态——同仓的 auth 与 compaction 把**恢复动作**
直接编码进类型：

- `AuthError` 加了 `#[non_exhaustive]`，好让未来加变体不破坏下游；但内层的
  `RefreshTokenError` **故意不加**——"永久失败 vs 瞬态失败"是每个调用方都必须穷举的
  封闭决策，漏一个就该编译报错（crates/codegen/xai-grok-shell/src/auth/error.rs:37）。
  同一份代码里两个相反的选择：外层为演进留口，内层为强制穷举关口。
- 更细的一手：瞬态错误**没有公开的 `From`**，只能经 `AuthError::transient` 构造
  （crates/codegen/xai-grok-shell/src/auth/error.rs:50）——防止一个随手写下的 `?` 把
  永久失败悄悄"升级"成可重试，把不可恢复误分类成可恢复。
- compaction 则把"相同输入重试还有没有意义"编码成 `is_deterministic()`
  （crates/common/xai-grok-compaction/src/sampler.rs:91）：调用方问的是一个语义谓词，
  而不是去 match 错误的 Display 字符串。

一条原则：**错误枚举首先服务于调用方的下一步动作（重试？换路？放弃？），其次才服务于
`Display`**——`#[non_exhaustive]` 的加与不加、`From` 的有与无，都是在为这个下一步划边界。
